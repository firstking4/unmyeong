...
type MenuHref = '/profile-edit' | '/gwansang' | '/saju-code-share' | '/history';
...
const MENU: MenuItem[] = [
  ...
  },
  {
    key: 'backup',
    title: '운명인지도 백업·복구',
    blurb: '비밀번호로 암호화해 저장·복원 (프로필·관상·지인·기록)',
    action: 'backup',
  },
];
...
export default function ModalScreen() {
  ...
  const runExport = async (password: string) => {
    ...
    setBusy(true);
    try {
      // PBKDF2가 메인 스레드를 막기 전에 로딩 UI가 그려지도록 한 프레임 양보.
  ...
  return (
    <ScrollView
      ...
      <View style={styles.themeBlock}>
        <Text style={[styles.themeLabel, { color: c.text }]}>화면 테마</Text>
      ...
      <View style={[styles.notificationBlock, { borderTopColor: c.hairline }]}>
        <View style={styles.notificationHeader}>
          <View style={styles.notificationCopy}>
            <Text style={[styles.themeLabel, { color: c.text }]}>오늘의 운세 알림</Text>
            <Text style={[styles.notificationHint, { color: c.muted }]}>
