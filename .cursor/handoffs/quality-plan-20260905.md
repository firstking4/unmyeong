...
**진행 (2026-09-05):** **S1-b 완료** — 키워드 표 확정(E·F 승인), saju·seonghyang·tarot·physiognomy·gunghap 팩 v2, `meta.json` version 2, `check:today-crosstab` 통과. 남은 S1 항목은 S1-c(칩 간격, 사용자 결정 대기)뿐. 결정 A~D 미확정. 다음은 S2.
...
4. ✅ `data/daily/meta.json` version 1 → 2 (순열이 새로 섞임). §9 전부 통과 → **S1-b 완료**. S1-c(칩 간격)는 아래 「그대로 둔 것」 참고, 사용자 결정 대기.
...
| **G** | S1-c — `pickDailyMany` 칩 3개를 이웃(p, p+1, p+2)에서 간격(p, p+8, p+16)으로 바꿀지 (날마다 칩 전부 교체, 8일 주기) | 바꾼다 — 칩 3개 중 2개가 전날과 겹치는 현재 구조가 「매일 같은 칩」으로 읽힌다 |
