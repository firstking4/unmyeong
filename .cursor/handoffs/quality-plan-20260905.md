...
**진행 (2026-09-05):** **S1 완료** · **S2 진행** — 결정 A·B를 「이어가」로 기본 제안 적용(헤드라인=허브 첫 칩, 본문 출처 라벨). `home` 팩(행동·주의)은 유지. 결정 C·D 미확정. 다음은 S2 검수 뒤 S5 또는 S4.
...
- **헤드라인 `moodHeadline`**: ✅ `{허브 첫 칩} · {등급}` (결정 A). 라운드로빈 첫 칸이 성향이라 주의 칩이 앞에 오면 「즉흥 · 주의」처럼 보일 수 있다.
...
| 결정 | 질문 | 기본 제안 |
|---|---|---|
| **A** ✅ | 지도 헤드라인 키워드 출처 — home 팩 유지 / 허브 첫 칩 / 「등급 · 톤」 | **허브 첫 칩 (2026-09-05, 「이어가」)** — `moodHeadline` = `{칩} · {등급}` |
