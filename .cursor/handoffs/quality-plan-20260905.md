...
**진행 (2026-09-05):** **S1~S6 구현 완료** (`feat/quality-s1-packs`). S6는 시뮬에서 배너·잠금 CTA·로딩 문구·테스트 해금 초기화(`__DEV__`만)를 확인. S5·S4·S3 문장 세부는 이후 화면에서 손봐도 됨. S3-c 옵션 세분화·S3-d 워프는 안 함.
...
- **`gunghap` 팩은 현재 `keyword`만 쓴다** (`lib/gunghap.ts` 1156행). 문장 필드는 죽은 데이터라 §1-1 측정에서 지인 충돌이 0이었다. 계획은 키워드만 새로 쓰는 것이었지만, v1 문장을 그대로 두면 `check:today-crosstab`의 「팩 간 동일 문장」이 `gunghap+home`으로 영원히 실패하므로 **문장 4종도 두 사람 행동으로 새로 썼다**(지인 풀이에 팩을 쓸 때 그대로 꺼내 쓸 수 있다). `home` 팩은 S2에서 지도 취합 방식이 정해진 뒤 다룬다 — home v1 문장은 이제 다른 팩과 겹치지 않는다.
...
- `check:today-content` 지인 5/14(「관심은」×4)은 팩이 아니라 십신 「관심」 템플릿(오늘 내 관심은 / 상대 관심은 / 올해 내 관심은 …)이 한 화면에 네 번 나오는 것. S2 또는 문구 정리 때 `lib/gunghap.ts`의 관심 문장 어미를 갈라 놓는다.
...
| 한 일 | S1 팩 분리 → S2 지도 취합·화면 검수 → S5 시드 두께 → S4 스프레드 → S3 관상 조합 → S6 UI(배너 아래·잠금·빈 프로필). 시뮬에서 S6 문구·테스트 해금 초기화 확인 |
...
- 사용자 수정 후 (이 커밋):  
  `오늘 흐름이 내 쪽으로 기울었습니다. 다만 이달·올해는 무리하지 않는 쪽이 편합니다. 점수는 69입니다.`  
  틀: 오늘 한 문장. 이달·올해 한 문장. `점수는 N입니다.`
