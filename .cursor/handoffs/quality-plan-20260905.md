...
**진행 (2026-09-05):** **S1~S6 구현 완료** (`feat/quality-s1-packs`). 같은 날 잔여 5항: MBTI×오늘 키워드 결 · home 팩 v2 · 지인 「관심은」 반복 제거는 반영. S3-c는 해설 준비 후. S3-d는 파이프 가능·잔상 미세조정은 향후.

새 채팅에서 이번 개선을 되돌아보려면 `@.cursor/handoffs/quality-plan-20260905.md` **§12**. 시나리오 설계는 §3~§8.
...
브랜치 `feat/quality-s1-packs`에서 S1 완료 후 S2 진행. `main`에는 오늘 카드 칩 통일·허브 이동·라이트 테마·검사 스크립트가 커밋돼 있다(`a6548e4`).
이 브랜치를 `main`에 합치는 것은 사용자 판단.
...
- 홈 순서: `HomeHeroFilled` → `IdentityCard` → `ShareBannerSlot` → `IntegratedFortune`/`LockedFortune` → **`AdBannerSlot`** → `DestinyQuote`. 광고 배너는 운세 카드 **아래**(결정 D).
...
| `physiognomy` | 인상·표정·태도: 눈맞춤 · 첫인상 · 표정 · 자세 · 미소 · 시선 · 목소리 · 여백 · 단정함 · 너그러움 · 끄덕임 · 말투 · 걸음 · 손짓 · 온화함 · 눈빛 · 옷차림 · 인사 · 웃음 · 침착함 · 다정함 · 기품 · 얼굴빛 · 어깨 펴기 | 「오늘 남에게 어떻게 보이는가」 — 부위 이름 없이 인상 행동. ✅ **헤드라인 중복 버그 수정** — `lib/physiognomy.ts`가 `${theme.keyword} · ${theme.headline}`로 붙여 「정리 · 정리 · 정리한 만큼…」이 됐다 → `theme.headline`만 |
...
- **`gunghap` 팩은 현재 `keyword`만 쓴다** (`lib/gunghap.ts` 1156행). 문장 필드는 죽은 데이터라 §1-1 측정에서 지인 충돌이 0이었다. 계획은 키워드만 새로 쓰는 것이었지만, v1 문장을 그대로 두면 `check:today-crosstab`의 「팩 간 동일 문장」이 `gunghap+home`으로 영원히 실패하므로 **문장 4종도 두 사람 행동으로 새로 썼다**(지인 풀이에 팩을 쓸 때 그대로 꺼내 쓸 수 있다). `home` 팩은 v2(2026-09-05) — 접두어 제거 · closing 24고유. 다른 팩과 문장 겹침 0.
- 정확한 사용 위치는 각 빌더에서 `theme.` 검색으로 재확인 (`rg "theme2?\.(headline|focus|relationship|action|caution)" lib`).

**작업 순서 · 진행**

1. ✅ 키워드 24개 표 확정 (위 표).
2. ✅ `saju` · ✅ `seonghyang` · ✅ `tarot` · ✅ `physiognomy`(헤드라인 중복 수정 포함) · ✅ `gunghap`(키워드 + 문장) 팩 v2 (모두 2026-09-05). 각 팩마다 화면 렌더 확인 → §9 검사 → 사용자 검수 → 커밋.
3. ✅ `cleanThemeLine` 삭제. ✅ `check:today-crosstab` 등록 — 스키마(24개 · 필드 누락 0 · 문장 끝 마침표 0 · 조사 병기 0 · keyword 팩 안 고유 · 네 탭 간 고유) + 팩 간 동일 문장 + 같은 날 교차 중복. **이제 통과 상태가 기준선** — 팩·빌더를 고칠 때마다 다시 돌린다.
4. ✅ `data/daily/meta.json` version 1 → 2 (순열이 새로 섞임). §9 전부 통과 → **S1-b 완료**. ✅ S1-c(칩 간격)는 「타로 단계에서 함께 고친 코드」 마지막 항목 참고.

**관상 단계에서 함께 고친 코드 (`lib/physiognomy.ts`)**

- 헤드라인 중복 버그 — `buildTodayPhysiognomy`가 `${theme.keyword} · ${theme.headline}`로 만들어 「정리 · 정리 · …」가 됐다(화면이 아니라 빌더 쪽). 팩 headline이 이미 「키워드 · 문장」이므로 `theme.headline`만 쓴다.
- salt는 `physiognomy:{선택 옵션 id}:{personSalt}`로 날짜 없음 — 그대로. 요약은 `focus` + 「오늘은 {부위}가 중심입니다. {옵션 summary}」, 힌트는 옵션 힌트 뒤에 `relationship`·`action`이 붙는 구조라 팩 문장은 **부위 이름 없이 인상·태도 행동**으로만 썼다(옵션 문장이 눈·코·입을 이미 말한다).
- 한 변주 안에서 키워드가 4~5번 메아리치던 것(「인사」×5 · 「얼굴」×5 · 「어깨」×5)을 2~3번으로 줄였다 — `check:today-content` 관상 3/14 → 0/14. 팩을 쓸 때 **키워드를 headline·focus·relationship·action·caution 다섯 칸에 다 넣지 않는다**.
- 남은 관상 반복은 시드 옵션 문장의 「편입니다」(`check:today-repetition` 14/14) — S3-b 시드 두께에서 다룬다.

**지인 단계 메모 (`lib/gunghap.ts`)**

- 팩은 `pickDaily('gunghap', gunghap:{pairSeed})`로 키워드만 칩에 들어간다. 코드 변경 없음.
- `check:today-content` 지인 5/14(「관심은」×4)은 팩이 아니라 십신 「관심」 템플릿이 한 화면에 네 번 나오던 것. 2026-09-05에 오늘/올해 후보를 「마음은 / 기울어요」로 갈라 「관심은」을 뺐다.
...
**성향 단계에서 함께 고친 코드 (`lib/seonghyang.ts`)**

- salt는 `west.id`(오늘 카드)·`mbti.id`(MBTI 레이어)로 날짜 없음 — 그대로. `buildTodayMbti`의 `reading` 문장은 UI에 안 그려진다(`MbtiPromptRow`는 ready면 null). 화면에 팩은 오늘 카드 한 번만 나와 블록 간 중복 없음.
- MBTI 강점 문장 템플릿을 「`{MBTI}`로는 ‘{키워드}’ 흐름 위에서 {강점} 쪽을 가볍게 살려 보세요」 → 「`{MBTI}`의 {강점}을 오늘의 ‘{키워드}’에 얹어 보세요」. 기질어(말수·온도·유머)에 「흐름 위에서」가 어색했다.
...
- ✅ **S1-c (결정 G, 2026-09-05)** — `pickDailyMany`가 이웃(p, p+1, p+2) 대신 `length / count` 간격(3개면 p, p+8, p+16 · `pickDailyPair`는 p, p+12)으로 뽑는다. 이전에는 오늘 `theme2`가 내일 `theme`이라 「어제 성장 힌트 = 오늘의 한 가지」가 매일 생기고 칩 3개 중 2개가 전날과 겹쳤다. 120일 시뮬레이션: 전날과 칩이 겹친 날 **116/119 → 2/119**(바퀴 경계의 우연), 8일 창 안 고유 칩 10~12 → 17~24. 같은 묶음은 8일 뒤 순서만 돌아 다시 오고 24일마다 새로 섞인다. 사주 `pickPeriodThemes`는 `3 + avoid.size`개를 받아 걸러 쓰므로 간격이 4~8로 달라지지만 결정적이라 문제없다.
...
진행 기록: 사주 팩 v2 뒤 교차 중복 21/60 → 20/60일, 사주가 낀 조합 0. 허브 고유 칩 30 → 54. 성향 팩 v2 뒤 16/60일(남은 조합 타로+관상 16 · 지도+관상 11 · 지도+타로 5), 허브 고유 칩 78. 타로 팩 v2 뒤 5/60일(남은 조합 지도+관상 11), 허브 고유 칩 102. 관상·지인 팩 v2 + meta v2 뒤 **0/60일 · 팩 간 동일 문장 0 · 스키마 통과** — `check:today-fixed` · `today-keywords-fixed` · `today-content`(관상 0/14) · `verify:fortune-variety` · `verify:daily-variety` 통과. **S1-b 완료 (2026-09-05).**
...
- 헤드라인 `moodHeadline`: ✅ `{등급 문장} · {허브 첫 칩}` (결정 A). 「무난한 하루 · 느긋함」처럼 왼쪽이 점수 등급, 오른쪽이 아래 칩과 같은 단어.
- **점수 근거 한 줄**: ✅ `buildScoreNote` — 「오늘 들어오는 기운이 … 48점입니다.」 상세(`scoreNote`)에 표시. 전문 코드 없음.
- **행동/주의**는 home 팩 유지 (종합 팩의 역할).
- 메모 키: ✅ `fortuneMemo` key에 `physiognomy` 선택 문자열.
- UI: ✅ 출처 라벨 muted 13pt, 누르면 해당 탭 (결정 B). 만세력 없을 때만 `introLine`을 라벨 없이 앞에 둔다.
- `check:today-crosstab` 화면 비교에서 지도의 성향·타로·관상 줄은 뺀다 (같은 빌더를 가져오는 것이 설계).

### 완료 기준

- 프로필이 완전할 때 본문에 성향·사주·타로·관상 각 1문장. 관상 미선택은 줄을 빼지 않고 「아직 얼굴 특징을 고르지 않았습니다」 안내(눌러 `/gwansang`).
...
| 항목 | 현재 | 제안 | 결정 |
|---|---|---|---|
| 광고 배너 위치 | 신분증 아래, 운세 카드 위 | 운세 카드 **아래** (`ads-plan.mdc` 「오늘의 운세 카드 아래」) | **D 적용 (2026-09-05 「진행해」)** |
| 지도 본문 출처 표기 | 없음 | S2 `sources` 라벨 + 탭 이동 | 결정 B |
| 빈 프로필 상태 | 탭 리드만 바뀜 | 오늘 카드 자리 `ProfileNeededCard` + 「신분증 채우기」. 지인·사주코드는 같은 문장+CTA | **적용** |
| 잠금 CTA 톤 | 아이콘만 · 「상세 풀이」 | 탭별 도메인 언어 버튼 (「오늘의 사주 풀이 보기」 등) | **적용** |
...
npm run check:seonghyang-grain       # 오늘 키워드 × MBTI 결 — INTJ×느긋함=장기 설계 상승, 시너지·가중 0
...
(2026-09-05 saju v2 뒤: 교차 중복 20/60일, 남은 조합 타로+관상 16 · 지도+관상 10 · 지도+타로 5 · 성향+타로 3 · 성향+관상 1. 팩 5개 v2 뒤: **0 — 통과.**)
...
| 결정 | 질문 | 기본 제안 |
|---|---|---|
| **A** ✅ | 지도 헤드라인 키워드 출처 | 기본안 `{칩} · {등급}` → 화면 검수 후 **`{등급 문장} · {칩}`** (`무난한 하루 · 느긋함`). §12-3 B #1 |
| **B** ✅ | 지도 본문에 출처 라벨(성향·사주·타로·관상)을 붙일지 | **붙임 (2026-09-05, 「이어가」)** — muted 13pt, 탭 이동 |
| **C** | 관상 조합 문장을 pairs 데이터로 / 축 규칙 템플릿으로 | 축 규칙 12문장부터, 부족하면 pairs. **「진행해」로 기본안 적용 (2026-09-05)** |
| **D** ✅ | 광고 배너를 운세 카드 아래로 옮길지 | **옮김 (2026-09-05 「진행해」)** — 핵심 콘텐츠 우선 |
| **E** ✅ | S1-b 도메인별 키워드 24개 표 (§3) | **승인됨 (2026-09-05)** — §3 표가 확정본 |
| **F** ✅ | 콘텐츠 작성(S1-b 600문장 · S4-a 234문장 · S5)을 AI 초안 → 사용자 검수로 진행할지 | **AI 초안, 팩 1개씩 검수** (S1-b 5팩 완료 · S4-a·S5에도 같은 방식) |
| **G** ✅ | S1-c — `pickDailyMany` 칩 3개를 이웃(p, p+1, p+2)에서 간격(p, p+8, p+16)으로 바꿀지 | **바꿈 (2026-09-05, 「이어가」로 기본 제안 진행)** — 전날과 겹친 날 116/119 → 2/119 |
...
시나리오 본문(§3~§8)은 **목표·설계**다. 이 절은 **실제로 무엇을 결정·수정했는지**다. 나중에 브랜치를 되돌아보거나 문구를 되돌릴 때 여기만 읽어도 된다.
...
| | 내용 |
|---|---|
| 왜 | 오늘 카드 문장이 팩 6개에서 같고, 지도 점수·본문·칩이 서로 다른 출처라 「취합」이 안 됐다 |
| 한 일 | S1 팩 분리 → S2 지도 취합 → S5·S4·S3 → S6 UI. 잔여: MBTI 결 문장 · home v2 · 지인 「관심은」 제거 |
| 안 한 일 | S3-c 이중 턱 분리(해설 준비 후) · S3-d 워프 잔상 미세조정(향후) · S5·S4·S3 문장 한 줄씩 화면 검수 |
| 다음 | 문장이 어색하면 그때 고친다. 이중 턱은 해설 카피 준비된 뒤. 워프는 파이프만 동작·굽기 안 함 |

**커밋 (이 브랜치, S1~S6)**

| 커밋 | 무엇 |
|---|---|
| `a565098` | S1-b 사주 팩 v2, salt에서 날짜 제거, 주·월·년 변주 중복 방지, `check:today-crosstab` |
| `9e134bb` | S1-b 성향 팩 v2, MBTI 강점·주의 템플릿 |
| `126c339` | S1-b 타로 팩 v2, `reverseHeadline`, 헤드라인에서 카드명 제거 |
...
| 결정 | 사용자가 한 말 | 결과 |
|---|---|---|
| **E** | 키워드 24개 표 승인 | §3 표가 확정본. 팩 문장은 AI 초안 → 팩 하나씩 검수 |
| **F** | 초안 쓰고 진행해 / 이어가 | S1-b 5팩 같은 방식. S4-a·S5에도 적용 |
| **G** | 「이어가」로 기본 제안 | 칩 이웃 → 간격 (p, p+8, p+16). `a1e596a` |
| **A** | 「이어가」로 기본 제안 후 **화면에서 수정** | 처음 `{칩} · {등급}` (`느긋함 · 무난`) → 사용자가 뜻을 모름 → `{등급 문장} · {칩}` (`무난한 하루 · 느긋함`). 12-3 #1 |
| **B** | 「이어가」 후 화면 확인 | 출처 라벨 + 탭 이동. **확정** (누르면 그 탭으로 감) |
| **C** | 「진행해」로 기본 제안 | 축 규칙 12문장 (`physiognomy-pairs.json`). 부족하면 pairs 추가 |
| **D** | 「진행해」로 기본 제안 | 배너를 운세 카드 아래로. `ads-plan.mdc` 갱신 |

권장 진행 순서는 사용자가 「권장 순서로 진행할께」라고 함: **S2 화면 확인 → S5 → S4 → S3 → S6**.

---

### 12-3. 사용자 수정·검수 (이번 작업에서 직접 고친 것)

S1 전에 지도·칩을 손본 것(`a6548e4`)과, S2 화면을 보고 다시 고친 것(이 커밋).

#### A. S1 직전 — 오늘 카드·허브 (이미 `main`/`a6548e4`)

사용자가 반복해서 요청한 것. 사후 검토 때 「칩이 왜 점수 아래인지」의 근거.

| 사용자 요청 | 반영 |
|---|---|
| 부정 키워드는 적어도 하루 하나. **꼭 하나일 필요는 없음** | `withSparseCaution` — 성향·사주·관상 닷새에 하루 주의 칩, 타로는 `reverseKeyword`로 매일 주의 칩 하나 |
| 운세 점수 키워드와 「오늘의 키워드」가 중복·따로 논다 | 점수 전용 칩 제거. 허브=`buildTodayKeywords`(성향·사주·타로·관상 오늘 카드와 같은 빌더) |
| 오늘의 키워드 **카드는 없애고**, 점수 키워드 있던 자리로 | 지도에 키워드 카드 없음. 칩은 점수 카드 안 |
| 안내 문구는 점수 옆, 칩은 점수 **아래**, 왼쪽 여백은 아래 문구와 같게 | `TodayKeywordCaption` + `TodayKeywords` |
| 키워드 누르면 관련 탭 | 칩·(S2 후) 출처 라벨 모두 이동 |
| 오늘 카드에 **고정 키워드 넣지 말 것**. 지인 궁합 칩도 허브에는 안 넣되 고정은 금지 | `today-keywords.mdc` · `check:today-keywords-fixed` |
| 관상·성향·사주에 칩이 하나만 보인다 / 날짜마다 바뀌는 칩이 더 있다 | 오늘 카드 칩 3개(타로도 3). 허브는 그 합 |
| 화면 테마 디폴트는 시스템이 아니라 **라이트** | `userInterfaceStyle: light` |

#### B. S2 1차 화면 확인 (2026-09-05, `eab0538` 뒤) — **이 커밋**

사용자가 지도를 보고 말한 것 그대로.

| # | 사용자가 본 것 | 사용자가 한 말 | 코드 | 상태 |
|---|---|---|---|---|
| 1 | 헤드라인 `느긋함 · 무난` / `즉흥 · 주의` | **헤드라인이 뭘 말하는 건지** | `GRADE_DAY`: 주의가 필요한 하루 · 조심할 하루 · 무난한 하루 · 좋은 하루 · 가장 좋은 하루. 형식 `{등급 문장} · {허브 첫 칩}` → `무난한 하루 · 느긋함` | 반영 · 커밋 |
| 2 | 출처 라벨 | **누르면 그 탭으로 가고 있음** | 유지 | **확정** |
| 3 | `조심스러워 69점입니다` | **말이 어색함**. 이전 표현을 물음 | 아래 이전/이후 | 반영 · 커밋 |
| 4 | 관상 미선택 시 줄이 사라짐 | **줄이 빠지지는 않고 선택이 안 되었다는 안내 문구가 있으면 됨** | `아직 얼굴 특징을 고르지 않았습니다. 누르면 골라 볼 수 있습니다.` `placeholder: true`, 눌러 `/gwansang` | 반영 · 커밋 |

**3번 점수 근거 — 같은 프로필·69점 날**

- S2 취합 **전**: 이 줄 없음.
- `eab0538` (사용자가 어색하다고 한 것):  
  `오늘 흐름이 내 쪽으로 기울고 이달·올해 흐름은 조심스러워 69점입니다.`  
  틀: `{기울고/잘 맞고/밀어 주고} {받쳐 주어/조심스러워/무난해} {N}점입니다.`
- 사용자 수정 후 (이 커밋):  
  `오늘 흐름이 내 쪽으로 기울었습니다. 다만 이달·올해는 무리하지 않는 쪽이 편합니다. 오늘의 점수는 69입니다.`  
  틀: 오늘 한 문장. 이달·올해 한 문장. `오늘의 점수는 N입니다.`

#### C. S6 UI (2026-09-05 「진행해」)

| 항목 | 반영 |
|---|---|
| 배너 위치 (D) | 홈: 운세 카드 아래 · `DestinyQuote` 위. 메뉴 하단은 유지. `ads-plan.mdc` |
| 잠금 CTA | 성향/사주/타로/지인 도메인 버튼 문구. 아이콘+텍스트 |
| 빈 프로필 | `ProfileNeededCard` (성향·사주·타로) · 지인·사주코드 CTA 「신분증 채우기」 → 지도 |
| 테스트 해금 초기화 | 설정 맨 아래. `__DEV__`(Expo Go)만. 스토어 빌드에는 없음 |
| 잠금 팝업 로딩 | `광고운은 오늘의 운세 점수와 연동됩니다.` / `잠시만 기다려 주세요.` |

시뮬에서 홈·잠금 팝업·설정 하단을 보고 문구를 다듬음.

### 12-4. 사후 검토 체크리스트 (나중에 볼 것)

화면을 다시 열고 아래만 보면 이번 개선의 체감이 남는지 판단할 수 있다.

- [ ] 지도 헤드라인이 `무난한 하루 · 느긋함`처럼 **등급이 문장으로** 읽히는가. 오른쪽 단어가 아래 칩에 있는가.
- [ ] 본문 사주·성향·타로·관상 라벨을 누르면 그 탭 오늘 카드와 **같은 결**이 이어지는가.
- [ ] 점수 근거가 `조심스러워 69점`이 아니라 문장이 나뉘어 읽히는가.
- [ ] 관상 특징을 지웠을 때 안내 줄이 남는가.
- [ ] 같은 날 성향/사주/타로/관상 오늘 카드 문장이 **서로 다른 어휘**인가 (`check:today-crosstab`).
- [ ] 칩이 전날과 거의 안 겹치는가 (S1-c 간격).
- [ ] 헤드라인 오른쪽이 주의 칩일 때 (`주의가 필요한 하루 · 즉흥`) 어색하지 않은가 — 12-3 #1과 겹침, 아직 안 봄.
- [ ] 지도에서 광고 배너가 운세 카드 **아래**, 명언 **위**인가.
...
| 항목 | 위치 | 왜 남았나 |
|---|---|---|
| S3-c 이중 턱 분리 | 관상 옵션 | **해설 카피가 준비된 뒤에만.** 지금은 옵션 id·워프 키·저장 selection을 바꾸지 않음. 형은 둥근/각진, 이중 턱은 독립 토글로 가기로 함 |
| S3-d 워프 잔상 | `gwansang:warps` | **가능 여부만 확인, 향후.** 파이프는 동작(국소 워프·경계 감쇠·얼굴형×턱 콤보 PNG·눈/입 복합·이중 턱은 Crease). 턱선 추적·옷깃·여성 가파른 턱 등 잔상 미세조정은 다시 굽지 않고 미룸. 나이·헤어 축은 범위 밖 |
| S5·S4·S3 문장 검수 | 시드 JSON | AI 초안 커밋됨. 화면에서 어색한 줄만 고치면 됨. 성향 MBTI 「얹어」는 결 문장으로 교체함 |
| S1-a 오프셋 | `pickDailySlots` | S1-b로 건너뜀. 다시 안 함 |

### 12-5. 잔여 5항 결정 (2026-09-05)

사용자가 화면 검수 뒤 방향을 줌.

| # | 말 | 반영 |
|---|---|---|
| 1 | 문장 검수는 꾸준히. 키워드 결은 상승 효과·역효과·무영향으로. 덧셈·뺄셈 비유는 안 씀. 음의 결은 부정 표현을 피하지 않음 | `keywordPairLines`. 시너지·가중·중립·덧셈·뺄셈 금지. `npm run check:seonghyang-grain` |
| 2 | 홈 팩 지도 변주 | `data/daily/packs/home.json` version 2. 접두어 「성향·사주·타로가 겹친 오늘에는」 삭제. closing 24고유. 키워드·headline id는 유지. 지도 행동·주의는 이 팩 |
| 3 | 지인 「관심은」 반복 줄이기 | `lib/gunghap.ts` 오늘/올해 후보에서 「관심은」 제거 |
| 4 | 이중 턱을 형과 분리 | 좋다고 함. **해설이 준비되면** 진행. 지금은 안 함 |
| 5 | 워프 잔상은 AI로 구현이 어려워 보임 | 지금 파이프가 가능한지만 확인. 새로 굽지 않음. 향후 개선 |
